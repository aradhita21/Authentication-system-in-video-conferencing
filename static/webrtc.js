    ...
    navigator.mediaDevices.getUserMedia(constraints)
      ...
      .then(() => {
        ...
        serverConnection.onopen = event => {
          serverConnection.send(JSON.stringify({ 'displayName': localDisplayName, 'uuid': localUuid, 'dest': 'all', 'source':'fromstreamer' }));
          console.log('from streamer');
        } 
      }).catch(errorHandler);
  ...
  if (navigator.mediaDevices.getUserMedia) {
    navigator.mediaDevices.getUserMedia(constraints)
      ...
      .then(() => {
        ...
        serverConnection.onmessage = gotMessageFromServer;
        serverConnection.onopen = event => {
          serverConnection.send(JSON.stringify({ 'displayName': localDisplayName, 'uuid': localUuid, 'dest': 'all','source':'fromclient' }));
...
function gotMessageFromServer(message) {
  ...
  if (signal.source == 'fromstreamer') {
    ...
    serverConnection.send(JSON.stringify({ 'displayName': localDisplayName, 'uuid': localUuid, 'dest': peerUuid , 'sending':'streamer'}));

    if (signal.source == 'fromclient') {
      // set up peer connection object for a newcomer peer
      setUpPeer(peerUuid, signal.displayName);
      serverConnection.send(JSON.stringify({ 'displayName': localDisplayName, 'uuid': localUuid, 'dest': peerUuid ,'sending':'client'}));
